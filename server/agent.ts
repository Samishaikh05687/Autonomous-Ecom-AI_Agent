  ...
  try {
    ...
    const itemLookupTool = tool(
      ...
      // Tool metadata and schema definition
      {
        ...
        description: "Gathers luxury bags item details from the Inventory database", // Description for the AI
      ...
      return retryWithBackoff(async () => { // Wrap in retry logic
        ...
        const prompt = ChatPromptTemplate.fromMessages([
          [
            ...
            `You are a helpful E-commerce Chatbot Agent for a luxury bags store. 

IMPORTANT: You have access to an item_lookup tool that searches the bags inventory database. ALWAYS use this tool when customers ask about bags items, even if the tool returns errors or empty results.

When using the item_lookup tool:
- If it returns results, provide helpful details about the bags items
